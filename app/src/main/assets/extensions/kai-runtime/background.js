(function () {
  ...
  browser.runtime.onMessage.addListener(async (message, sender) => {
    ...
    if (declaredLength > MAX_RESPONSE_BYTES) throw new Error("Network response exceeds 32 MB");
    const responseBytes = new Uint8Array(await response.arrayBuffer());
    if (responseBytes.length > MAX_RESPONSE_BYTES) throw new Error("Network response exceeds 32 MB");
